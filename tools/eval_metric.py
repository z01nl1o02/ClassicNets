#https://raw.githubusercontent.com/zhreshold/mxnet-ssd/master/evaluate/eval_metric.py
...
class MApMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds): #4-dims (1,1,n,5) or (1,1,n,6)
        """
            ...
            while (pred.shape[0] > 0):
                ...
                if gts.size > 0:
                    ...
                    for j in range(dets.shape[0]):
                        ...
                        ovargmax = np.argmax(ious) #max-iou of one det wrt gts
                        ovmax = ious[ovargmax]
    ...
    def _recall_prec(self, record, count):
        ...
        tp = np.cumsum(sorted_records[:, 1].astype(int) == 1) #sorted by score from high to low
